...
## 目录
- [消保智审助手（CPIR）](#消保智审助手cpir)
  - [目录](#目录)
  - [更新日志](#更新日志)
  - [1 背景简介](#1-背景简介)
  - [2 快速上手](#2-快速上手)
    - [2.1 项目主要结构](#21-项目主要结构)
    - [2.2 运行步骤](#22-运行步骤)
    - [2.3 运行样例](#23-运行样例)
  - [3 产品测试](#3-产品测试)
    - [3.1 思考样例](#31-思考样例)
      - [豆包的思考](#豆包的思考)
      - [deepseek的思考](#deepseek的思考)
    - [3.2 信息源样例](#32-信息源样例)
    - [3.3 模型对比](#33-模型对比)
    - [3.4 方案设计](#34-方案设计)
  - [4 搭建智能体](#4-搭建智能体)
    - [4.1 主流程](#41-主流程)
      - [4.1.1 拒答判断（reject）](#411-拒答判断reject)
      - [4.1.2 规划（planing）](#412-规划planing)
      - [4.1.3 行动（action）](#413-行动action)
      - [4.1.4 反思（reflection）](#414-反思reflection)
      - [4.1.5 循环（loop）](#415-循环loop)
    - [4.2 认知工具](#42-认知工具)
      - [4.2.1 工具池设计](#421-工具池设计)
      - [4.2.2 实践经验](#422-实践经验)
    - [4.3 检索工具](#43-检索工具)
  - [5 其他运行样例](#5-其他运行样例)
  - [6 优化思路](#6-优化思路)
...
- 2025-01-05：v1.1 版本发布
  - 新增分析节点（analyse node），一定程度上可以提高规划有效性；
  - 修复之前和反思混在一起的小bug，优化了规划模块的输出；
  - PS：录视频挺麻烦就不更新了；
<div align="center">
<img 
...
│     └── 📄 Understand.py
├── 📂 output/ # 输出目录
│  ├── 📄 知识问答_案例审核追问.md # qwen2.5-72b 输出日志
│  ├── 📄 文案审核.md # qwen3-8b 输出日志
│  └── 📄 知识问答.md # qwen3-8b 输出日志
├── 📂 picture/ # 图片目录
│  ├── 📄 graph.png # 任务执行流程图
├── 📂 old/ # 旧脚本
│  ├── 📄 Casa_QA_v1.0.py
...
可以看到，模型在理解问题后，通过调用认知工具和检索工具，查询到了相关的制度文件，最后根据文件内容，输出了审核结果。
<div align="center">
  <img 
    src="./video/qwen2p5_72b_qa1.gif" 
    alt="样例" 
    style="max-width: 800px; height: auto; border: 2px solid #5a5858ff; border-radius: 8px; padding: 5px;"
  >
</div>

然后追问分红险文案审核，可以看到agent根据历史上下文识别到了相关性和差异性：
...
<div align="center">
  <img src="./video/qwen2p5_72b_qa2.gif"
    alt="样例" 
    style="max-width: 800px; height: auto; border: 2px solid #5a5858ff; border-radius: 8px; padding: 5px;"
  >
</div>

上下文理解还是有些不足，没有重新查询新的规则，而是根据历史上下文的信息来审核了。具体执行细节可参考```output/知识问答_案例审核追问.md```
...
### 4.1 主流程
见```Case_QA.py```，里面是agent的主要结构，主要包括拒答判断、规划、行动、反思4个步骤。
<div align="center">
